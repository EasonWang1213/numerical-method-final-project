...
class LMSFilter:
    ...
    def adapt(self, desired, input_signal):
        ...
        return output, error
當然，這裡是最終版的 `README.md`，包含詳細的安裝和使用說明：
...
LMS-NoisePurify/
├── data/
│   ├── input_signal.wav       # Noisy audio file
│   ├── noise_signal.wav       # Pure noise file
│
├── filters/
│   ├── __init__.py            # Filter module initializer
│   ├── lms_filter.py          # LMS filter implementation
│
...
1. Haykin, S. (1996). Adaptive Filter Theory (3rd ed.). Prentice Hall.
2. Widrow, B., & Stearns, S. D. (1985). Adaptive Signal Processing. Prentice Hall.
3. Ifeachor, E. C., & Jervis, B. W. (2002). Digital Signal Processing: A Practical Approach (2nd ed.). Prentice Hall.

---

This `README.md` provides a clear and visually appealing guide for installing and using the LMS NoisePurify project. It ensures that other developers can easily understand the project structure, dependencies, and usage instructions.
```

### `requirements.txt`

確保您的 `requirements.txt` 文件如下所示：
